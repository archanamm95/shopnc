...
class ControllerAccountRegister extends Controller {
	...
	public function error()
	{
		...
		echo $this->language->get('error_telephone');
		exit();

		$error['error_firstname']=$this->language->get('error_exists');
	...
	public function index() {
		...
		if (isset($this->request->get['purchase'])) {
	        ...
	        unset($this->session->data['referral_session']);
	        $data['purchase_session']=$decoded_sponsor_purchase;
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->session->data['register_data'] = $_POST;

			//START CHECK IF EMAIL EXIST IN MLM 
			$url= HTTP_MLMSERVER.'check-email-exist/'.$_POST['email'];
	        $curl=curl_init($url);
	        curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
	        curl_setopt($curl, CURLOPT_POST, false);
	        curl_setopt($curl, CURLOPT_SSL_VERIFYPEER, false);
	   ...
	   //      	$this->response->addHeader('Content-Type: application/json');
	        	...
	        	$this->response->redirect($this->url->link('product/register'));
	        }
	        //START CHECK IF EMAIL EXIST IN MLM 

			$customer_id = $this->model_account_customer->addCustomer($this->request->post,$data['referral_session'],$this->session->data['type']);
			...
			$this->response->redirect($this->url->link('product/all_product'));
		}
