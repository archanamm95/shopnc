	...
	public function autocomplete() {
		...
		if (isset($this->request->get['filter_name'])) {
			...
			foreach ($results as $result) {
				...
				if ($result['image']) {
						$image = $this->model_tool_image->resize($result['image'], 85, 110);
					} else {
						$image = $this->model_tool_image->resize('placeholder.png', 85, 110);
					}
				...
				else
				{
					...
					if(!empty($purcahsed_share_product))
					{
						...
						if($purcahsed_share_product == "false")
						{
							$json['products'][] = $product_array;
						}
