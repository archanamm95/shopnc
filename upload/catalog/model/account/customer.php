...
class ModelAccountCustomer extends Model {
	public function addCustomer($data,$sponsor=MLM_COMPANY,$type) {

		$username="NC".rand(11111,99999);
	...
	public function addCustomer2($data) {
		...
		$this->db->query("UPDATE " . DB_PREFIX . "customer SET  custom_field = '" . $this->db->escape(isset($data['account']) ? json_encode($data['account']) : '') . "' WHERE customer_id = '". (int)$customer_id ."'
			");
	...
	public function editCustomer($customer_id, $data) {
		...
		$data['username']=$this->customer->getEmail();
        $ch = curl_init(HTTP_MLMSERVER.'profile-update');
	...
	public function updateUsername($customer_id,$username) {

		$this->db->query("UPDATE " . DB_PREFIX . "customer SET  username = '" . $username . "' WHERE customer_id = '". (int)$customer_id ."'
			");
