...
class Customer {
  ...
  public function login($email, $password, $override = false) {
		...
		{
			$customer_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer WHERE username = '" . $this->db->escape(utf8_strtolower($email)) . "' AND (password = SHA1(CONCAT(salt, SHA1(CONCAT(salt, SHA1('" . $this->db->escape($password) . "'))))) OR password = '" . $this->db->escape(md5($password)) . "') AND status = '1'");
		}
		 else {
